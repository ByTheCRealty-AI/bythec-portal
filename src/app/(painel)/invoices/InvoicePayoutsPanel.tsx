...
// como a By the C pagou o OWNER e o CLEANER (método) + recibo opcional.
// "Owner pago" REUSA o flag `paid` do invoice (mesma ação Mark as paid).
// "Cleaner pago" usa cleaner_paid. Recibos aqui têm category owner_payout /
// cleaner_payout — NÃO entram no PDF combinado (guest). Escondido na impressão.
// =============================================================================

import { useRef, useState, useTransition } from "react";
import { createClient } from "@/lib/supabase/client";
import { buttonClass } from "@/components/ui";
import { Wallet, Check, RotateCcw, Upload, Loader2, FileText, Trash2, ExternalLink } from "lucide-react";
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        {paid ? (
          <span className="inline-flex items-center gap-1 rounded-full border border-primary/25 bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary">
            <Check className="h-3 w-3" /> Paid{paidAtLabel ? ` · ${paidAtLabel}` : ""}
          </span>
        ) : (
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <select
            ...
            className="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm text-ink outline-none transition focus:border-primary/40 focus:ring-2 focus:ring-primary/15 disabled:opacity-60"
          >
    ...
    <div className="print-hide mt-6 rounded-2xl border border-black/[0.08] bg-white p-5 shadow-card">
      ...
      <div className="space-y-3">
        <PayoutRow
          ...
          markPaidLabel="Mark owner as paid"
          markUnpaidLabel="Mark owner as unpaid"
        />
        ...
        {cleaningToBythec && (
          <PayoutRow
            ...
            markPaidLabel="Mark cleaner as paid"
            markUnpaidLabel="Mark cleaner as unpaid"
          />
        )}
      </div>
    </div>
